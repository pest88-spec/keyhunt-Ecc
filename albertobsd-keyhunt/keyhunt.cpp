...
while ((c = getopt(argc, argv, "deh6MgqRSB:b:c:C:E:f:I:k:l:m:N:n:p:r:s:t:v:G:8:z:")) != -1) {
		switch(c) {
			case 'h':
				menu();
			break;
case 'g':
				FLAGGPU = 1;
				fprintf(stderr,"[+] GPU backend requested.\n");
			break;
			case '6':
				FLAGSKIPCHECKSUM = 1;
	...
	// Check GPU availability after getopt parsing
	if (FLAGGPU) {
		if (!GPU_IsAvailable()) {
			fprintf(stderr, "[E] GPU backend requested but no CUDA device available.\n");
			FLAGGPU = 0;
		} else {
...
void *thread_process(void *vargp)	{
	...
	// Dynamic batch size: GPU uses larger batches, CPU uses fixed size
	uint32_t effective_batch_size = FLAGGPU ? GPU_BATCH_SIZE : CPU_GRP_SIZE;

	// Heap allocation for dynamic batch sizes (避免栈溢出)
	Point *pts = new Point[effective_batch_size];
	Point *endomorphism_beta = new Point[effective_batch_size];
	Point *endomorphism_beta2 = new Point[effective_batch_size];
	Point endomorphism_negeted_point[4];

	Int *dx = new Int[effective_batch_size / 2 + 1];
	IntGroup *grp = new IntGroup(effective_batch_size / 2 + 1);
	Point startP;
	Int dy;
	Int dyn;
	Int _s;
	...
	Point pp;
	Point pn;
	int i,l,pp_offset,pn_offset;
	int hLength = (effective_batch_size / 2 - 1);  // Dynamic based on batch size
	uint64_t j,count;
	...
	// GPU mode: pre-allocate batch buffers once per thread (避免重复 malloc/free)
	uint32_t *h_priv_le = nullptr, *h_pubx_le = nullptr, *h_puby_le = nullptr;
	if(FLAGGPU) {
		
		h_priv_le = (uint32_t*)malloc(GPU_BATCH_SIZE * 8 * sizeof(uint32_t));
		h_pubx_le = (uint32_t*)malloc(GPU_BATCH_SIZE * 8 * sizeof(uint32_t));
		h_puby_le = (uint32_t*)malloc(GPU_BATCH_SIZE * 8 * sizeof(uint32_t));
		if(!h_priv_le || !h_pubx_le || !h_puby_le) {
			fprintf(stderr, "[E] GPU batch malloc failed, thread %i\n", thread_number);
			if(h_priv_le) free(h_priv_le);
			if(h_pubx_le) free(h_pubx_le);
			if(h_puby_le) free(h_puby_le);
			exit(EXIT_FAILURE);
		}
	}
	...
	do {
		...
		if(continue_flag)	{
			...
			// GPU-first batch mode (M1 milestone requirement)
			if(FLAGGPU) {
				// Use pre-allocated buffers (allocated at line 2575-2577)
				// Pack batch private keys: base_key + i*stride
				Int temp_key;
				for(uint32_t i = 0; i < GPU_BATCH_SIZE; i++) {
					temp_key.Set(&key_mpz);
					if(i > 0) {
						Int temp_offset;
						temp_offset.SetInt32(i);
						temp_offset.Mult(&stride);
						temp_key.Add(&temp_offset);
					}
					// Convert Int to uint32_t[8] LE (Int uses NB64BLOCK=5, bits stored in bits64[])
					for(int j = 0; j < 8; j++) {
						if(j < 8) {
							uint64_t limb64 = (j/2 < NB64BLOCK) ? temp_key.bits64[j/2] : 0ULL;
							h_priv_le[i*8 + j] = (j%2 == 0) ? (uint32_t)(limb64 & 0xFFFFFFFFULL) : (uint32_t)(limb64 >> 32);
						} else {
							h_priv_le[i*8 + j] = 0u;
						}
					}
				...
				// Call GPU batch computation (GPU-only strategy per INTEGRATION_PLAN_CN.md line 178)
				int rc = GPU_BatchPrivToPub(h_priv_le, h_pubx_le, h_puby_le, GPU_BATCH_SIZE, 256);
				if(rc != 0) {
					fprintf(stderr, "[E] GPU_BatchPrivToPub failed (code %d), thread %i. GPU is core, exiting.\n", rc, thread_number);
					exit(EXIT_FAILURE);
				}

				// Convert GPU results to Point format and feed into hash matching
				for(uint32_t i = 0; i < GPU_BATCH_SIZE; i++) {
					// Convert uint32_t[8] LE to Int (affine x, y coordinates)
					Point gpu_point;
					for(int j = 0; j < 4; j++) {  // Only need first 4 uint64 for 256-bit
						uint64_t x_limb = ((uint64_t)h_pubx_le[i*8 + j*2]) | (((uint64_t)h_pubx_le[i*8 + j*2 + 1]) << 32);
						uint64_t y_limb = ((uint64_t)h_puby_le[i*8 + j*2]) | (((uint64_t)h_puby_le[i*8 + j*2 + 1]) << 32);
						gpu_point.x.bits64[j] = x_limb;
						gpu_point.y.bits64[j] = y_limb;
					}
					gpu_point.x.bits64[4] = 0ULL;  // Clear extra limb
					gpu_point.y.bits64[4] = 0ULL;
					gpu_point.z.SetInt32(1);  // Affine point: z=1

					pts[i] = gpu_point;
				}

				// Buffers will be reused in next iteration (freed at thread exit)
				// Now pts[] contains GPU-computed public keys, process hash matching
				// Note: GPU mode processes full batch at once, no inner loop needed
				count = GPU_BATCH_SIZE;  // Mark batch as processed
			}

			// Original CPU point group algorithm (only when FLAGGPU == 0)
			do {
				if(FLAGGPU) break;  // GPU mode skips CPU point group
				...
				*/
				...
				for(j = 0; j < hash_batch_limit; j++){
					...
					// CPU mode: increment by 4 keys per inner loop iteration
					if(!FLAGGPU) {
						count+=4;
				...
				*/
				...
				// CPU mode: update startP for next iteration
				if(!FLAGGPU) {
					steps[thread_number]++;
			...
			// GPU mode: update key for next batch
			if(FLAGGPU) {
				Int temp_gpu_stride;
				temp_gpu_stride.SetInt32(GPU_BATCH_SIZE);
				temp_gpu_stride.Mult(&stride);
				key_mpz.Add(&temp_gpu_stride);
				steps[thread_number]++;
			}
		}
	} while(continue_flag);
	...
	delete[] pts;
	delete[] endomorphism_beta;
	delete[] endomorphism_beta2;
	delete[] dx;
	delete grp;
...
printf("-g          use GPU backend (CUDA + KEYHUNT-ECC)\n");
